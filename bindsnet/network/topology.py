...
class Connection(AbstractConnection):
    ...
    def normalize(self) -> None:
        ...
        def sorting_2(target):
            sec1 = (target >= 0) * (target < 0.1)
            sec2 = (target >= 0.1) * (target < 0.2)
            sec3 = (target >= 0.2) * (target < 0.3)
            sec4 = (target >= 0.3) * (target < 0.4)
            sec5 = (target >= 0.4) * (target < 0.5)
            sec6 = (target >= 0.5) * (target < 0.6)
            sec7 = (target >= 0.6) * (target < 0.7)
            sec8 = (target >= 0.7) * (target < 0.8)
            target[sec1] = 0.05
            target[sec2] = 0.15
            target[sec3] = 0.25
            target[sec4] = 0.35
